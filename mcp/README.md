...
| **Get a forest inventory number for an area** | Read FIADB-API's "under construction" docs, guess at 752 attribute codes and 1,129 evaluation ids, build the query by hand | `search_fia` finds the attribute/grouping/evaluation offline from bundled catalogs; `fia_estimate` returns the estimate WITH its sampling error, and flags cells too thin to report |
| **Summarize land cover or change over an area** | Find the LCMS service, work out its parameter encoding, parse the response | `lcms_summary` returns land cover, land use, or change areas for a state/county/forest — no authentication, no Earth Engine required |
