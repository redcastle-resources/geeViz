...
> **Note (v2026.8.1):** Several tool names referenced in results below have been consolidated: `get_api_reference` → `search_functions(function_name="...")`, `extract_and_chart` → `cl.summarize_and_chart()` in `run_code`, `get_thumbnail` → `tl.generate_thumbs()` in `run_code`, `list_examples`/`get_example` → `examples(action="list"|"get")`. Historical results are kept as-is.
